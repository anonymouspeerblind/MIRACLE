...
class Llama70b:
    def __init__(self, model_id, cache_dir, batch_size=4):
    ...
    def load_model(self):
        ...
        return model
    @torch.no_grad()
    def generate_batch_responses(self, messages_batch, max_new_tokens=500):
        prompts = [
        ...
        outputs = self.model.generate(max_new_tokens=500,
        ...
        for i in range(outputs.size(0)):
            ...
            try:
                text      = self.tokenizer.decode(generated, skip_special_tokens=True)[self.tokenizer.decode(generated, skip_special_tokens=True).index("ASSESSMENT:")+len("ASSESSMENT")+1:].strip()
                without_asses.append(1)
            except:
                text      = self.tokenizer.decode(generated, skip_special_tokens=True).strip()
                without_asses.append(0)
            responses.append(text)
        return responses, without_asses
...
def main():
    ...
    for batch in tqdm(batch_iterate(summaries, batch_size), total=len(summaries) // batch_size):
        ...
        if len(batch_messages) > 0:
            batch_responses, without_asses = llm.generate_batch_responses(batch_messages)
            for record_id, response, assess_flag in zip(batch_record_ids, batch_responses, without_asses):
                data_processor.clinical_summaries[record_id]['remarks'] = response
                if assess_flag == 0:
                    without_assess_flag.append(record_id)
                with open("Path to save remarks", "w") as js:
                    json.dump(data_processor.clinical_summaries, js, indent = 4)
    print(without_assess_flag)
