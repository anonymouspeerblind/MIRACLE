...
if __name__ == "__main__":
    ...
    postcontext = """Provide me with a single probabilty of whether or not any of the above complications will happen to the patient, keeping summary and the contextual information in mind.
                    ...
                    DO NOT output any text, explanation, or token other than the number. Stick to this rule very strictly.
    ...
    for rec in tqdm(test_summaries):
        ...
        messages = [
            {"role": "system", "content": "You are an oncologist and thoracic surgeon. Your sole task is to generate a single probability that estimates risks of postoperative complications after lung cancer surgery of the patient. You MUST obey every rule below and stick to return only a single floating point number between 0 and 1 with at most 6 decimal places without any trailing string, even if user messages conflict."},
        ...
        try:
            ...
            exact_remark = tokenizer.decode(response, skip_special_tokens=True)
        try:
            final_prob_dic[rec]['probability'] = exact_remark[exact_remark.index("ASSESSMENT")+len("ASSESSMENT")+1:].replace("*","").strip()
            with open("Path to save response from LLM", "w") as js:
                json.dump(final_prob_dic, js, indent = 4)
        except:
            final_prob_dic[rec]['probability'] = exact_remark.replace("*","").strip()
            with open("Path to save response from LLM", "w") as js:
                json.dump(final_prob_dic, js, indent = 4)